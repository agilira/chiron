 ...
 * 
 * Universal media player for HTML5 video and audio elements
 * Supports progressive enhancement with custom controls
 * 
 * Features:
 * - Play/Pause with spacebar and K
 * - Skip forward/backward (±10s with J/L, ±5s with arrows)
 * - Volume control with M for mute
 * - Fullscreen with F (video only)
 * - Keyboard accessible
...
class LumosPlayer {
  constructor(wrapperId) {
    ...
    this.media = this.wrapper.querySelector('video') || this.wrapper.querySelector('audio');
    
    if (!this.media) {
      console.error('Media element not found in', wrapperId);
      return;
    }
    ...
    this.volSlider = null;
    this.timeDisplayCurrent = null;
    this.timeDisplayDuration = null;
  ...
  renderControls() {
    ...
    // Skip buttons (solo Video)
    const skipBtnsHTML = isVideo ? `
      <!-- Skip Back (-10s) -->
      <button class="lumos-rwd-btn" aria-label="Rewind 10 seconds">
         <div>
            ${ICONS.rewind10}
            <span>10</span>
         </div>
      ...
      <!-- Play/Pause -->
      <button class="lumos-play-btn" aria-label="Play">
        ${ICONS.play}
      </button>

      <!-- Skip Forward (+10s) -->
      <button class="lumos-fwd-btn" aria-label="Forward 10 seconds">
         <div>
            ${ICONS.forward10}
            <span>10</span>
         </div>
    ...
    const fsBtnHTML = isVideo ? `
       <button class="lumos-fs-btn" aria-label="Enter Fullscreen">
         ${ICONS.fullscreen}
       </button>` : '';

    // Caption Button (solo Video)
    const ccBtnHTML = isVideo ? `
        <button class="lumos-cc-btn" aria-label="Captions (Not available)">
            ${ICONS.cc}
        </button>` : '';
    ...
    const controlsHTML = `
      ...
      <div class="lumos-controls">
        
        <!-- Progress Bar Area -->
        <div>
          <!-- Background Track -->
          <div>
            <div class="lumos-progress-fill"></div>
          </div>
          
          <!-- Accessible Range Input -->
          <input type="range" min="0" max="100" step="0.1" value="0" 
            class="lumos-range lumos-time-slider" 
        ...
        <div>
          <div>
            ...
            <!-- Volume Group -->
            <div class="lumos-volume-group">
              <button class="lumos-mute-btn" aria-label="Mute">
                ${ICONS.volumeHigh}
              </button>
              <div class="lumos-volume-slider-container">
            ...
            <!-- Time -->
            <div class="lumos-time-display" aria-hidden="true">
              <span class="lumos-time-current">0:00</span>
              <span>/</span>
  ...
  bindEvents() {
    ...
    if (!this.isAudio) {
      ...
      // Per Audio: Click su intero wrapper attiva Play/Pause
    ...
    this.wrapper.addEventListener('keydown', (e) => this.handleKey(e));
  }
  ...
  updateProgressVisuals(current, duration) {
...
// Auto-initialize all Lumos players on page load
document.addEventListener('DOMContentLoaded', () => {
  const players = document.querySelectorAll('[data-lumos-player]');
  
  players.forEach(wrapper => {
    try {
      new LumosPlayer(wrapper.id);
    } catch (e) {
      console.error(`Failed to initialize Lumos Player for ${wrapper.id}:`, e);
    }
  });
  
  if (players.length > 0) {
    console.log(`Initialized ${players.length} Lumos Player(s) 🎬🎵`);
  }
});
